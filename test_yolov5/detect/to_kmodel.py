...
def parse_model_input_output(model_file,input_shape):
    ...
    for _, e in enumerate(input_tensors):
        ...
        # Use helper.tensor_dtype_to_np_dtype for newer ONNX versions
        try:
            input_dict['dtype'] = onnx.helper.tensor_dtype_to_np_dtype(onnx_type.elem_type)
        except AttributeError:
            # Fallback for older ONNX versions
            try:
                mapping = onnx.mapping if hasattr(onnx, 'mapping') else onnx._mapping
                input_dict['dtype'] = mapping.TENSOR_TYPE_TO_NP_TYPE[onnx_type.elem_type]
            except:
                # Final fallback: use int32 as default
                input_dict['dtype'] = np.int32
